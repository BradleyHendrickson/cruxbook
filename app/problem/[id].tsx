...
export default function ProblemDetailScreen() {
  ...
  const fetchProblem = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('problems')
      .select('id, name, description, avg_grade, vote_count, boulder_id, height, style, first_ascent_name, first_ascent_date')
      .eq('id', id)
      .single();
    setProblem(data);
    setLoading(false);
  ...
  if (!id) {
    return (
      <View style={styles.center}>
        <Text style={styles.text}>Invalid problem</Text>
      </View>
    );
  }
        ...
        <View style={styles.menuContainer}>
          <Pressable
            ...
            onPress={() => {
              ...
              router.push({
                pathname: '/boulder/[id]',
                params: { id: problem.boulder_id, sectorId, areaId, sectorName, areaName },
              });
            }}
          >
            <FontAwesome name="cube" size={16} color={Colors.dark.tint} />
            <Text style={styles.menuItemText}>View boulder</Text>
          </Pressable>
          <Pressable
          ...
          >
            ...
            <Text style={styles.menuItemText}>Edit problem</Text>
          </Pressable>
        </View>
      </Pressable>
    </Modal>
  ...
  return (
    <>
      ...
      >
        <View style={styles.header}>
          ...
          {problem.vote_count > 0 && (
            <Text style={styles.voteCount}>
              {problem.vote_count} vote{problem.vote_count !== 1 ? 's' : ''}
            </Text>
          )}
        </View>

        {(areaName || sectorName || boulderName) && (
          <Text style={styles.breadcrumb}>
            {[areaName, sectorName, boulderName].filter(Boolean).join(' › ')}
          </Text>
        )}

        {(problem.height || problem.style) && (
          <View style={styles.metaRow}>
            {problem.height && (
              <Text style={styles.metaItem}>Height: {problem.height}</Text>
            )}
            {problem.style && (
              <Text style={styles.metaItem}>Style: {styleToLabel(problem.style)}</Text>
            )}
          </View>
        )}

        {problem.first_ascent_name && (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>First ascent</Text>
            <Text style={styles.description}>
              {problem.first_ascent_name}
              {problem.first_ascent_date ? ` (${problem.first_ascent_date})` : ''}
            </Text>
          </View>
        )}
          ...
          <View style={styles.actionRow}>
            ...
            >
              <FontAwesome
                ...
                size={18}
                color={isFavorite ? '#e74c3c' : Colors.dark.text}
              />
            </Pressable>
          </View>
        )}
                  ...
                  onPress={async () => {
                    ...
                    if (newRating != null) {
                      const { error } = await supabase.from('route_ratings').upsert(
                        { user_id: user.id, problem_id: id, rating: newRating },
                        { onConflict: 'user_id,problem_id' as const }
                      );
        ...
        <Pressable
          style={styles.backToBoulder}
          onPress={() =>
            router.push({
              pathname: '/boulder/[id]',
              params: {
                id: problem.boulder_id,
